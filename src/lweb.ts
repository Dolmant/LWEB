...
const cleanUpSelectedItems = () => {
    ...
    // remove any generated elements from selected items
    for (const el of document.getElementsByClassName('temp')) {
      el.remove()
    }
}
  ...
  switch(true) {
    ...
    case (source.file.includes(".gltf")): {
      newImg.addEventListener("click", function() {
        ...
        cleanUpSelectedItems()
        if (!alreadySelected) {
          // Todo kill scene when something else is selected
          // newVideo.className = 'temp'
...
const loadItems = (forcePopulate: typeof selected = "") => {
  const category = forcePopulate || selected
  const parent = document?.getElementById(category) as HTMLElement;
  const columnEl = parent.children[0]

  let i = 0
  while (true) {
    if (!config[category][i]) {
      break
    }
    loadAsset(columnEl, config[category][i])
    i++
  }
}
