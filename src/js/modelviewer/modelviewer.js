...
class Viewer {
  constructor(el, options, THREE, dat, Stats, xhr) {
    this.dat = dat;
    this.xhr = xhr;
    this.Statser = Stats;
    this.el = el;
  ...
  resize() {
    const { clientHeight, clientWidth, width, height } = this.el.parentElement;
  ...
  addGUI() {
    const gui = (this.gui = new this.dat.GUI({ autoPlace: false, width: 260 }));
  ...
  constructor(el, options, THREE, dat, Stats, xhr) {
    ...
    this.options = {
      ...
      cameraPosition: options.cameraPosition
        ? options.cameraPosition.split(",").map(Number)
        : null
    };
