  ...
  pJS.fn.interact.linkParticles = function(p1, p2){
    ...
    if(dist <= pJS.particles.line_linked.distance){
      ...
      if(opacity_line > 0){  
        ...
        const distModifier = Math.min(0.5, (100 - dist)/5000)
        if (distModifier > 0 && distModifier) {
          const p1vx = p1.vx * distModifier + p2.vx * (1 - distModifier)
          const p1vy = p1.vy * distModifier + p2.vy * (1 - distModifier)
          const p2vx = p2.vx * distModifier + p1.vx * (1 - distModifier)
          const p2vy = p2.vy * distModifier + p1.vy * (1 - distModifier)
          p1.vx = p1vx
          p1.vy = p1vy
          p2.vx = p2vx
          p2.vy = p2vy
